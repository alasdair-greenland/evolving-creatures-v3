...
// THIS FILE IS A NOISE FUNCTION GENERATOR
// It's used for map generation. I copy-pasted the entire file
// from online. don't mess with it. thx. Are there going to be noises?
...
function addNoiseProperty(global) {
...
addNoiseProperty(game.temperature);
addNoiseProperty(game.moisture);
addNoiseProperty(game.elevation);
addNoiseProperty(game.variance);

game.temperature.noise.seed(Math.random());
game.moisture.noise.seed(Math.random());
game.elevation.noise.seed(Math.random());
game.variance.noise.seed(Math.random());